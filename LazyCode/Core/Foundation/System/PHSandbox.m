...
+ (NSString *)appPath
{
    return [[PHSandbox sharedInstance] appPath];
}
...
+ (NSString *)docPath
{
    return [[PHSandbox sharedInstance] docPath];
}
...
{
    return [[PHSandbox sharedInstance] libPrefPath];
}
...
{
    return [[PHSandbox sharedInstance] libCachePath];
}
...
+ (NSString *)tmpPath
{
    return [[PHSandbox sharedInstance] tmpPath];
}
...
{
    return [[PHSandbox sharedInstance] touch:path];
}
...
{
    return [[PHSandbox sharedInstance] touchFile:file];
}
...
+ (void)createDirectoryAtPath:(NSString *)aPath{
    if ( NO == [[NSFileManager defaultManager] fileExistsAtPath:aPath isDirectory:NULL] )
    {
        ...
        if ( NO == ret )
        {
            PHLog(@"%s, create %@ failed", __PRETTY_FUNCTION__, aPath);
            return;
        }
    }
}
